...
function testStyle (styleObj) {
  return /[0-9]+/.test(styleObj.zIndex) ||
  styleObj.opacity < 1 ||
  (styleObj.transform && styleObj.transform !== 'none') ||
  (styleObj.transformStyle && styleObj.transformStyle !== 'flat') ||
  (styleObj.transformBox && styleObj.transformBox !== 'border-box') ||
  (styleObj.filter && styleObj.filter !== 'none') ||
  (styleObj.perspective && styleObj.perspective !== 'none');
}
/**
...
export default class Element {
  ...
  constructor ({
    ...
    this.node = node;
    this.realNode = node; // 会出现作用域冲突，导致值被莫名替换的bug
   ...
   */
  scrollManually () {
    const dom = this.document.getElementsByClassName('common-content-zone')[0]; // 就这样写死吧，不想往里面传了，这组件！！！！！！
    if (dom.scrollTop) { // 其他页面scrollTop不为0时，必须手动置顶，不然会导致计算不准确
      dom.scrollTo(0, 0);
    }
    const elementRect = this.realNode.getBoundingClientRect();
  ...
  bringInView () {
    ...
    if (!this.realNode || this.isInView()) {
      return;
    }
    ...
    // scrollIntoView会扰乱布局，所以统一采用scrollTo
    this.scrollManually();
  ...
  fixStackingContext () {
    ...
    while (parentNode) {
      ...
      let styleObj = {zIndex, opacity, transform, transformStyle, transformBox, filter, perspective};
      // Stacking context gets disturbed if
      // - Parent has z-index
      // - Opacity is below 0
      // - Filter/transform or perspective is applied
      if (testStyle(styleObj)) {
