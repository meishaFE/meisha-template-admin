...
export default class Popover extends Element {
  ...
  show (position = 'bottom') {
    ...
    this.setPopperTitle();
    this.setPopperWidth(this.options.width);
  ...
  renderFooter () {
    ...
    let showPrevButtons = this.options.showPrevButtons;
    if (!showPrevButtons) {
    ...
    if (!this.options.showCloseButtons) this.closeBtnNode.style.display = 'none';

    if (!this.options.showNextButtons) {
    ...
    this.renderFooterBtn(showPrevButtons);
    this.renderFooterOfFirstStep(showPrevButtons);
    this.renderFooterOfLasStep();
  }

  renderFooterOfLasStep () {
    this.nextBtnNode.innerHTML = this.options.isLast
      ? this.options.doneBtnText : this.options.nextBtnText;
  }
  renderFooterOfFirstStep (showPrevButtons) {
    if (this.options.isFirst) {
      if (showPrevButtons) this.prevBtnNode.classList.add(CLASS_BTN_DISABLED);
      this.nextBtnNode.innerHTML = this.options.startBtnText;
    } else {
      if (showPrevButtons) this.prevBtnNode.classList.remove(CLASS_BTN_DISABLED);
    }
  }

  renderFooterBtn (showPrevButtons) {
    const hasSteps = this.options.totalCount && this.options.totalCount !== 1;
    // If this is just a single highlighted element i.e. there
    if (!hasSteps) {
      // this.footerNode.style.display = 'none';
      // this.nextBtnNode.style.display = 'none';
      this.nextBtnNode.classList.add(PRIMARY_BTN);
    } else {
      // 如果只有“下一步” 按钮这它是主按钮
      if (showPrevButtons) {
        this.prevBtnNode.style.display = 'inline-block';
        this.prevBtnNode.classList.add(PRIMARY_BTN);
      } else {
        this.nextBtnNode.classList.add(PRIMARY_BTN);
      }
      // @todo modify CSS to use block
      if (this.options.showNextButtons) this.nextBtnNode.style.display = 'inline-block';
    }
  }
  /**
